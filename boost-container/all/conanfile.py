...
class BoostContainerConan(ConanFile):
    ...
    topics = ("boost", "containers", "data", "time")
    ...
    def requirements(self):
        ...
        # transitive headers:boost/container/set.hpp:15
        self.requires(f"boost-config/{self.version}", transitive_headers=True)
        # transitive headers: boost/container/node_handle.hpp:34
        self.requires(f"boost-assert/{self.version}", transitive_headers=True)
        # transitive headers: boost/container/detail/value_functors.hpp:21
        self.requires(f"boost-intrusive/{self.version}", transitive_headers=True)
        # transitive headers: boost/container/detail/addressof.hpp:21
    ...
    def generate(self):
        ...
        tc.cache_variables["CMAKE_PROJECT_boost_container_INCLUDE"] = os.path.join(self.source_folder, os.pardir, "conan_project_include.cmake")
        tc.generate()
        deps = CMakeDeps(self)
        deps.generate()        
    ...
    def package_info(self):
        self.cpp_info.set_property("cmake_file_name", "boost_container")
        self.cpp_info.set_property("cmake_target_name", "Boost::container")
        self.cpp_info.libs = ["boost_container"]
        self.cpp_info.defines = [
            "BOOST_CONTAINER_NO_LIB",
            "BOOST_CONTAINER_DYN_LINK" if self.options.shared else "BOOST_CONTAINER_STATIC_LINK"
            ]
